          ...
          thrustDelay                 = 0.6;           /// initial delay to cause lesser slip when on 1st gear - thrust goes from zero to full in this time
          brakeIdleSpeed              = 1.78;      /// under what speed (in m/s) does the brake apply for a vehicle without thrust
          maxSpeed                    = 70;           /// vehicle can go a bit over, but dramatically decreases thrust
          fuelCapacity                = 45;
          wheelCircumference          = 3.233;      /// diameter of 725
          //wheelRadius -> 0.525

          antiRollbarForceCoef     = 1;      /// how strong is the anti-roll bar of vehicle preventing it to lose grip in turns (not any magical stuff, real ARB)
          antiRollbarForceLimit     = 5;      /// highest possible force of ARB
          ...
          idleRpm = 1200; // RPM at which the engine idles.
          redRpm = 6900; // RPM at which the engine redlines.

          class complexGearbox
          {
               GearboxRatios[]    = {"R1",-10.094,"N",0,"D1",5.562,"D2",3.456,"D3",2.623,"D4",1.678,"D5",1.078};
               TransmissionRatios[] = {"High",5}; // Optional: defines transmission ratios (for example, High and Low range as commonly found in offroad vehicles)
               ...
               driveString        = "Towards Da Enamy"; // string to display in the HUD for forward gears.
               neutralString      = "No Change"; // string to display in the HUD for neutral gear.
               reverseString      = "French Mode"; // string to display in the HUD for reverse gears.
          ...
          torqueCurve[] = {{0, 0.2}, {0.278, 0.5}, {0.35, 0.85}, {0.461, 1}, {0.7, 0.95}, {0.75, 0.85}, {0.8, 0.7}, {1, 0.5}};
          ...
          class Wheels
          {
               class LF
               {
                    ...
                    springDamperRate = 20000; // springDamperRate = 1.0 * 2 * sqrt(50000 * 2000)
